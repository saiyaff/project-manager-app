...
class MasterViewController: UITableViewController, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate {
    ...
    func insertNewObject(_ sender: Any) {
        ...
        self.preparePopover(contentController: viewController, sender: self.addButton, delegate: self)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        configureCell(cell, withEvent: project)
        return cell
    }
    ...
    func configureCell(_ cell: UITableViewCell, withEvent project: Projects) {
        cell.textLabel!.text = project.projectName!.description
    }
    ...
    var fetchedResultsController: NSFetchedResultsController<Projects> {
        ...
        let sortDescriptor = NSSortDescriptor(key: "projectDueDate", ascending: false)
    ...
    func pushNotificationsTrigger() {
        let fetchRequest: NSFetchRequest<Tasks> = Tasks.fetchRequest()
        let filterPredicate = NSPredicate(format: "taskReminderFlag =[c] %@", NSNumber(value: true))
        fetchRequest.predicate = filterPredicate
        do {
            let allNotifyingTasks = try PersistenceService.context.fetch(fetchRequest)
            if (allNotifyingTasks.count > 0) {
                for task in allNotifyingTasks {
                    let taskDueDate = task.taskDueDate! as Date
                    let currentDate = Date()
                    let completedStatus = task.taskPercentage
                    if (completedStatus < 100) && currentDate > taskDueDate {
                        NotificationService.showNotification(projectName: task.parentProject?.projectName! ?? "", taskName: task.taskName!)
                    }
